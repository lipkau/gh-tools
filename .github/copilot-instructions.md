...
- **Testing**: Jest framework with TypeScript support (21 test suites)
- **Linting**: ESLint (flat config) + Prettier + markdownlint
- **Build**: TypeScript compiler with separate configs for type-checking and compilation
- **Package Manager**: npm
...
- **Action-specific tsconfigs**: Actual compilation to `lib/` directory
...
- **Test files**: Place in `tests/` directories with `.test.ts` extension
...
**Prevention**: Keep source (`.ts`) and output (`.js`) in separate directories
...
npm test         # Should pass all 21 tests
```
...
- `lib/` and `dist/` directories (compiled output)
...
4. **Keep source and output separate**: TypeScript source in `src/`, compiled output in `lib/`
